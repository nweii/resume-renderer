...
- `lib/schema.ts` — the Zod schema. This is the contract between agent-written content and the renderer. `docs/schema-contract.md` is its generated, agent-facing statement — the authoritative content contract to read before writing resume content. Regenerate it with `bun run cli contract` after any schema change; `check` fails while it is stale. `docs/markdown-dialect.md` states the markdown notation for the same shape; the contract links to it.
- `lib/resume-variants.ts` — the one place that defines public URL paths. A variant's content file is `.json` or `.md`; a `.md` import is the file's text (the `[loader]` table in `bunfig.toml` for Bun, the `*.md` rule in `next.config.ts` plus `scripts/text-loader.js` for Turbopack, `lib/markdown-modules.d.ts` for TypeScript).
- `lib/resume-content.ts` — reads a variant's content file into validated content, or into issues stated in that file's terms (a JSON path, or a line and heading). The page, the data endpoints, and `check` all go through it.
- `lib/resume-markdown.ts` — the markdown dialect, both directions: `resumeToMarkdown` behind the `.md` endpoints, and `parseResumeMarkdown`, its inverse, behind markdown content files. Both read the schema for what each kind holds; `sectionKindLabels` is the one table there that knows what a label means. `lib/frontmatter.ts` is the YAML subset it uses for the header. A template overrides the writer with `toMarkdown` only when its outline differs, and gives up the round trip when it does.
...
- `cli/` — the `resume` CLI, built on [incur](https://github.com/wevm/incur). `index.ts` mounts commands; each command is a folder that exports a `register` function, so one that grows subcommands becomes its own group without touching the root. `check/` validates registered variants against the schema (a markdown variant's problems come back by line and heading), enforces the changelog contract, fails when `docs/schema-contract.md` is stale, and fails when `package.json`'s version lags the newest changelog release. `contract/` regenerates that document from the schema. `update/` is for downstream copies: it fetches the `upstream` remote and prints the changelog section of every release not yet reviewed, oldest first, for the operating agent to judge and port — it never applies anything. `--reviewed <tag>` records the review (adopted or declined) in `.upstream-reviewed`, a gitignored per-copy dotfile. `setup` seeds that file with the release the copy was created from (the newest release heading in its own `CHANGELOG.md`), and a first `update` on a copy without one seeds it the same way, so a fresh copy starts current. Upstream's tags are fetched into `refs/remotes/upstream/tags/*`, never into the copy's own tag namespace. Being behind upstream is information, not an error, so every `update` path exits zero. `variant/` manages the document collection: `variant create <slug>` scaffolds a registered, rendering variant (`--markdown` makes its content file markdown in the dialect; `--template` binds a template, `--theme` a theme, which defaults to the template's id), and `variant list` reads the registry back; deletion is manual (see "Variants" in README.md). `setup/` is the guided first run: it detects what a copy still lacks (Cloudflare auth, a Worker name in `wrangler.jsonc`, a first deploy), runs only the missing stages behind confirmation gates, and prints the live workers.dev URL; rerunning it reports state and repairs, never duplicates, and every prompt is answerable by a flag (`--name`, `--yes`, `--skip-deploy`) so an agent can drive it. `deploy/` holds the two publishing commands: `deploy` builds and runs `wrangler deploy` — the explicit production deploy — and `preview` builds and runs `wrangler versions upload`, printing the staged version's preview URL without changing what production serves. An unauthenticated run of either fails naming the way in (`wrangler login`, or `CLOUDFLARE_API_TOKEN` with Workers Scripts: Edit). `doctor/` diagnoses the whole environment read-only — variants, contract freshness, `wrangler.jsonc`, Cloudflare auth, the Worker's existence, upstream staleness — with a named fix per problem; network checks skip with a note when offline, and being behind upstream is information, exit zero. Run it with `bun run check`, or `bun run cli <command>`. Every command reads the repo at the working directory's git root (`repo.ts`: `repoPath` for files, `loadRepoModule` for that repo's registry, schema, content reader, and templates), never the checkout the CLI's own source was loaded from, so this repo's CLI run inside a copy examines the copy.
...
- A registered variant must be tracked in git. `.gitignore` keeps `resumes/*.json` and `resumes/*.md` untracked, and the build imports every registered variant. Register one, and add its un-ignore line in the same change, or a fresh clone fails to build.
- The validation error page is the feedback surface. A failure renders one line per problem, located in the content file's own terms: a JSON path, or a line and heading for markdown. Read that page instead of adding logging.
- `--format` is incur's own option on every command (output format), so no command may declare one. `variant create` takes `--markdown` for that reason.
...
Markdown in the dialect (`docs/markdown-dialect.md`) is not a working copy. A variant whose content file is `resumes/<slug>.md` is edited in place: the file is canonical content, the renderer parses it, and nothing reconciles. When a user wants to write markdown, offer to switch the variant's content file to markdown (`/resume.md` of the variant is already the file, byte for byte) rather than keeping a markdown mirror of JSON. A working copy is for every other surface.
...
| --- | --- | --- | --- |
| _(slug)_ | _(docx, cloud doc, …)_ | _(path or doc link)_ | _(mirror / regenerated)_ |

A variant with no row has no working copy: its content file is what gets edited. Add a row as the user declares a preference.

### Lifecycle rules

- **Canonical content wins on conflict.** When a working copy and the variant's content file disagree and you cannot tell which edit is newer, regenerate the working copy from the content file and tell the user what you replaced. Never guess the content file into agreement with a stale copy.
...
- **Nothing outside the declared home.** In-repo working copies live in `working/`, named by variant slug. Do not write mirrors, drafts, or scratch conversions of variant content anywhere else in the repo. A copy outside the repo exists only where the table declares it. Markdown in the dialect lives in `resumes/` as a content file, never in `working/`.
...
A release is an annotated tag on `main`. Read the version off the unreleased section: one breaking kernel entry makes it a minor bump, and everything else is a patch. The release commit turns `## Unreleased` into the version heading and sets `version` in `package.json` to the same number; `check` fails while they disagree, and `resume --version` reads that field. A breaking release also migrates the demo content, so the port note has a worked example beside it.
