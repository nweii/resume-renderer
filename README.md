...
4. **Site identity** — [`lib/site.ts`](lib/site.ts) holds the canonical name / title / description and favicon colors (consumed by `app/layout.tsx` and `app/icon.tsx`). Mirror the name into the filename strings in [`public/_headers`](public/_headers) so the `/resume.json` and `/resume.md` endpoints get matching `Content-Disposition` filenames in production.
